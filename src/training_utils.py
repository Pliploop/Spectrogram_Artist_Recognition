...
class RecoNetTrainer:
        ...
        for epoch in t:
            ...
            for x_batch, y_batch in self.train_batches:
                ...
                batch_accuracy.append(self.accuracy(torch.argmax(
                    self.model(x_batch), dim=-1), torch.argmax(y_batch, dim=-1)))
            ...
            with torch.no_grad():
                ...
                for x_val, y_val in self.val_batches:
                    ...
                    batch_val_acc.append(self.accuracy(torch.argmax(
                        self.model(x_val), dim=-1), torch.argmax(y_val, dim=-1)))
            ...
            t.refresh()
        torch.save(f'{datetime.datetime.now}_{training_accuracy*100}')
    ...
    def evaluate(self, batch_size=1):
        ...
        with torch.no_grad():
            predictions = []
            values = []
            for x_test, y_test in self.test_batches:
