...
class Splitting_Pipeline():
    ...
    def __init__(self,config) -> None:
        ...
        columns = ['artist','song','album','year','genre','uuid','duration','rms','path']
        self.unsplit_dataset =pd.DataFrame(columns=columns)
        

    def split_song(self,current_path,target_path,song):
        try:
            song_id = self.unsplit_dataset[self.unsplit_dataset.path == song].uuid.iloc[0]
            if not os.path.exists(target_path+'/'+song[0:len(song)-4]+'/'+'vocals.mp3'):
                self.separator.separate_to_file(current_path, destination=target_path, synchronous=False, bitrate='320k', codec='mp3')
            print(song)
            while not os.path.exists(target_path+'/'+song[0:len(song)-4]+'/'+'vocals.mp3'):
                time.sleep(1)
                
            if not os.path.exists(self.split_pool+f'{song_id}.mp3'):
                shutil.copyfile(target_path+'/'+song[0:len(song)-4]+'/'+'vocals.mp3',self.split_pool+f'{song_id}.mp3')
        except:
            pass
    ...
    def split_dataset(self):
        for artist in os.listdir(self.unsplit_path):
            ...
            self.split_folder(self.unsplit_path+f'{artist}',self.split_path+f'{artist}')
    ...
    def build_unsplit_dataset(self):
        ...
        for artist in os.listdir(self.unsplit_path):
            ...
            for k in tqdm(range(len(os.listdir(self.unsplit_path+f"{artist}/")))):
                ...
                self.unsplit_dataset=self.unsplit_dataset.append(pd.DataFrame([feature_row],columns=self.unsplit_dataset.columns))
        if len(self.unsplit_dataset)>0:
            self.unsplit_dataset.year=self.unsplit_dataset.year.apply(lambda x:x[0:4])
            self.unsplit_dataset.genre=self.unsplit_dataset.genre.apply(simple_genre)
